...
const createSeedData = async () => {
  try {
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      const estimatedDelivery = new Date(createdAt.getTime() + Math.random() * 7 * 24 * 60 * 60 * 1000); // Random delivery within 7 days
       ...
       const parcel = new Parcel({
         ...
         status: randomStatus,
         history: history,
         weight: (Math.random() * 20 + 0.1).toFixed(2), // 0.1 to 20.1 kg
         dimensions: {
           length: Math.floor(Math.random() * 50) + 10, // 10-60 cm
           width: Math.floor(Math.random() * 40) + 8,   // 8-48 cm
      ...
      await parcel.save();
      parcels.push(parcel);
      console.log(`📦 Parcel ${i} created for ${randomUser.name} - ${trackingNumber}`);
    }
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      console.log(`📦 Admin parcel ${i} created - ${trackingNumber}`);
    }
    ...
    console.log('\n📋 Login Credentials:');
    console.log('Admin: admin@parceltracker.com / admin123');
    console.log('Users: any email above / password123');
